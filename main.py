...
print('Wzór uzyskanego wielomianu:') 
interpolation.designate_polynomial().show_function()
'''
"""
SZUKANIE DNI DLA TEMPERATUR UJEMNYCH ORAZ DODATNICH UŻYWAJĄC METOD PRZYBLIŻONYCH - TUTAJ METODY STYCZNYCH
""" 
'''
...
for scope in scopes:
    a, b = scope
    method = NRMethod(polynomial, a, b)         # instancja klasy z met. stycznych
    approx = method.iterate(eps=10**(-5))       # iteracja do wyniku z przyjętą dokładnością zwracająca tablicę kolejnych przybliżeń
...
# z ujemnej na dodatnią i na odwrót 
...
#wyznaczenie pochodnej tego wielomianu
derivative = polynomial.designate_derivative()

#na bazie wykresu wyznaczam przedział, w którym szukane będzie ekstremum lokalne
...
x = approx[-1]
print('Znalezione x = {} to maximum lokalne funkcji'.format(x) if derivative.designate_res_for_x(x-0.01) > 0 
                                                    else 'Znaleziony x = {} to minimum lokalne funkcji'.format(x))

# szacuje konkretny dzień kryjący się pod znalezionym ekstremum
...
# szukanie najlepszego parametru n
for n in range(1, 21):
...
print('Średnia dobowa temperatura w roku w Krakowie wynosiła ok.', round(average_temp, 2), 'stopni Celsjusza')
'''
