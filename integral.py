...
class Integral:
    ...
    def solution(self):     # metoda, która zgodnie ze wzorem oraz wartościami podanymi w konstruktorze
        h = self.h          # liczy całke oznaczoną
        ys = self.ys
        alphas = self.alphas
        mul = lambda a,b: a*b
        return (3/8)*h*sum(list(reduce(mul, [y, ap]) for y, ap in zip(ys, alphas))) # suma iloczynów par alfa*y pomnożona przez h*(3/8)
